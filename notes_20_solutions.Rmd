...
```{r mr_1}
mr_1 <- lm(log(price) ~ age ^ 2 + car, data = sports_car_prices)
tidy(mr_1)
```
...
Using `geom_smooth()` an interaction effect is included in the fitted model
between `age` and `car`.
...
```{r mr_2}
mr_2 <- lm(price ~ age + car + age:car, data = sports_car_prices)
tidy(mr_2)
```
...
```{r mr_3}
mr_3 <- lm(price ~ age * car, data = sports_car_prices)
tidy(mr_3)
```
...
```{r}
augment(mr_2) %>% 
  summarise(R2 = 1 - (sum((.resid) ^ 2)) / (var(price) * 59))
```
...
Ideally, we want to see no pattern in the the plot as we increase the
index value.
...
The independence assumption is not violated for this dataset.

### Equal Variance and linearity check

- Equal variance: we want to see constant variability in the residual values
  across the fitted values.
  
- Linearity: we want to see the points scattered on both sides of 0 with no
  visible pattern
...
- If linearity assumption is violated, try transformations on your x variables

- If equal variance assumption is violated, try transformations on your y
  variable (1 / y or log(y))

### Normality check

Histogram:

Ideally, we want to see a bell-shaped and symmetric distribution centered at 0.

```{r normality_check_1}
ggplot(mr_1_aug, mapping = aes(x = .resid)) +
  geom_histogram(binwidth = 10000, fill = "pink", color = "grey90") + 
...
- If normality is violated, try a tranformation on your y variable (1 / y or
  log(y))
...
Our interaction term is a function of other predictors in the model, so of
course there will be some correlation between an interaction term and its
individual effects.
...
```{r mr_4}
mr_4 <- lm(price ~ age + mileage, data = sports_car_prices)
tidy(mr_4)
```
...
```{r mr_5}
mr_5 <- lm(Ozone ~ Solar.R + Wind + Temp, data = airquality)
tidy(mr_5)
```
...
```{r mr_5_aug}
mr_5_aug <- augment(mr_5)
```

```{r mr_5_independence_check}
p1 <- ggplot(mr_5_aug, aes(x = seq(nrow(mr_5_aug)), y = .resid)) + 
  geom_point() + 
  labs(x = "Index", y = "Residual value") +
  theme_minimal()
```
...
```{r mr_5_var_lin}
p2 <- ggplot(mr_5_aug, mapping = aes(x = .fitted, y = .resid)) +
  geom_point() + 
  geom_hline(yintercept = 0, color = "red", lty = 2) +
  labs(x = "Predicted Price", y = "Residual value") +
  theme_minimal()
...
```{r mr_5_normality_1}
p3 <- ggplot(mr_5_aug, mapping = aes(x = .resid)) +
...
```{r mr_5_normality_2}
p4 <- ggplot(mr_5_aug, mapping = aes(sample = .resid)) +
  stat_qq() + 
  stat_qq_line() +
  theme_minimal()
```
